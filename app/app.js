(function(){
  ...
  app.controller('Article', ['$http', '$scope', '$routeParams', 'marked', function ($http, $scope, $routeParams, marked) {
    $scope.article = {
      "title": "加载中…"
    };
    $http({
      method: 'GET',
      url: '/article/config/' + $routeParams["title"] + '.json'
    }).success(function (data, status, headers, config) {
      $scope.article = data;
      $http({
        method: 'GET',
        url: '/article/content/' + $routeParams["title"] + '.md'
      }).success(function (data, status, headers, config) {
